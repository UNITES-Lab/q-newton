...
from models.utils import load_and_freeze_pretrained_embedding_head_for_mistral
...
def train_mistral_tiny_from_scratch(
    ...
    model = load_and_freeze_pretrained_embedding_head_for_mistral(
        model,
        pretrained_model_name_or_path=pretrained_model_name
    )

    # optimizer
    optimizer = AdamW(model.parameters(), lr=learning_rate, weight_decay=weight_decay) if use_adam else SGD(
    ...
    wandb.init(
        project='qnewton',
        name='train-mistral-tiny-from-scratch' + (
            '-with-newton' if enable_newton else '') + f'-{normalization_epsilon}',
    )
...
if __name__ == '__main__':
    Fire(train_mistral_tiny_from_scratch)
